...
elseif (isset($_GET['get_user'])) { FetchData(trim($_GET['get_user']), $conn); } 
...
function UpdateData($user_id, $key, $values, $conn)
{
    if (!empty($keys) && !empty($values)) {
        ...
        if ($alreadyExists > 0) {
            $sqlArr = [];
            foreach ($jsonToArray as $key => $val) {
                $sqlArr[] = "$key = '$val'";
            }
            ...
            if ($results == true && $results > 0) {
                ...
                echo json_encode(['status' => 'Failed', 'message' => 'Data Not Updated!']);
            }
        } else {
            echo json_encode(['status' => 'Failed', 'message' => 'User id not found!']);
        }
    }
}
    ...
    if (!empty($keys) && !empty($values)) {
        ...
        if ($alreadyExists > 0) {
            ...
            if ($results == true && $results > 0) {
                ...
                echo json_encode(['status' => 'Failed', 'message' => 'Data Not Deleted!']);
            }
        } else {
            echo json_encode(['status' => 'Failed', 'message' => 'User id not found!']);
        }
...
function FetchData($user_id, $conn)
{
    ...
    if ($conn->query($query)->num_rows > 0) {
        ...
        echo json_encode(['status' => 'Failed', 'data' => 'Data Not Found']);
    }
}
